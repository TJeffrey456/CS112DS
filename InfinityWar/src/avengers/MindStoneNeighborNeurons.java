 ...
 *     //Call StdOut.println() for EVERY neuron (vertex) neighboring the Mind Stone neuron (vertex)
 *  
...
public class MindStoneNeighborNeurons {
    ...
    public static void main (String [] args) {
        ...
        HashMap<Integer,String> neurons = new HashMap<>();
        for(int i=0;i<numOfNeurons;i++){
            neurons.put(i, StdIn.readString());
        }
        ...
        String mindstone = neurons.get(neurons.size()-1);
        int synapses = StdIn.readInt();
        String connected = "";
        //looking for mindstone
        for(int j=0;j<synapses;j++){
            //noticed how target is always the second one
            connected = StdIn.readString();
